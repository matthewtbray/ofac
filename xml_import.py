...
_DDL_TABLES = """\
CREATE TABLE [{s}].[SDN_PublishInfo] (
    [ID]            INT      NOT NULL IDENTITY(1,1),
    [Publish_Date]  DATE     NOT NULL,
    [Record_Count]  INT      NOT NULL,
    [Ingested_At]   DATETIME NOT NULL DEFAULT GETDATE(),
    CONSTRAINT [PK_SDN_PublishInfo] PRIMARY KEY ([ID])
);
GO
...
def main():
    ...
    root = tree.getroot()
    ns   = _get_ns(root)
    find, _, text_fn = _make_finders(ns)

    # Extract <publshInformation> (OFAC typo: missing 'i')
    pub_el       = find(root, "publshInformation")
    publish_date = text_fn(pub_el, "Publish_Date") if pub_el is not None else None
    record_count = text_fn(pub_el, "Record_Count") if pub_el is not None else None
    if publish_date:
        print(f"  SDN Publish_Date: {publish_date}  Record_Count: {record_count}")
    else:
        print("  Warning: <publshInformation> not found in XML.")
    ...
    with pyodbc.connect(conn_str) as conn:
        ...
        # Insert publish metadata
        if publish_date:
            try:
                from datetime import datetime
                pd_parsed = datetime.strptime(publish_date, "%m/%d/%Y").date()
                rc_int    = int(record_count) if record_count else 0
                cursor.execute(
                    f"INSERT INTO [{s}].[SDN_PublishInfo] (Publish_Date, Record_Count)"
                    f" OUTPUT INSERTED.ID VALUES (?, ?)",
                    [str(pd_parsed), rc_int]
                )
                pub_id = cursor.fetchone()[0]
                print(f"  SDN_PublishInfo ID={pub_id}  "
                      f"Publish_Date={pd_parsed}  Record_Count={rc_int}")
            except Exception as exc:
                print(f"  Warning: could not insert SDN_PublishInfo: {exc}")
